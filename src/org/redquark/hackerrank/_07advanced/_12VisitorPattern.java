 ...
 *
 * In other words, new functionality should be added by writing an extension for the
 * existing code rather than modifying it and potentially breaking other code that uses it.
 ...
 * so the tree's root has depth  and each descendant node's depth is equal to the depth of its parent node .
 *
 * In this challenge, we treat the internal implementation of the tree as being closed to modification,
 * so we cannot directly modify it; however, as with real-world situations, the implementation is written
 * in such a way that it allows external classes to extend and build upon its functionality.
 * More specifically, it allows objects of the TreeVis class (a Visitor Design Pattern) to visit the tree
 * and traverse the tree structure via the accept method.
 *
 ...
 *       stored in the tree's non-leaf nodes at even depth and the sum of values stored in the tree's
 *       green leaf nodes. Recall that zero is an even number.
 *
 ...
 *    the getResult method returns the correct result for the implementing class' visitor.
 *
 * Part II: Read and Build the Tree
 * Read the -node tree, where each node is numbered from 1 to n. The tree is given as a list of node values
 * (x1, x2, .... , xn), a list of node colors (c1, c2, ..., cn), and a list of edges.
...
public class _12VisitorPattern {
